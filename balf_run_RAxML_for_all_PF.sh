...
## USAGE: ./balf_run_RAxML_for_all_PF.sh <email>

## Alignment file: balf_clade_simData.phy
## PF partitions file: balf_clade_simData.PF.partitions


######################################## START ###########################################
echo "INFO      | $(date) | Starting balf_run_RAxML_for_all_PF analysis... "
			...
			(
				if [[ -f ./PartitionFinder/analysis/best_scheme.txt  ]] && [[ "$(wc -c ./PartitionFinder/analysis/best_scheme.txt | sed 's/\ .*//' | perl -pe $'s/\t//g')" -gt 10000 ]]; then 
					cd ./raxml/; 
						...
						if [[ ! -f ./balf_clade_simData.phy ]]; then 
							cp ../ElConcatenero/balf_clade_simData.phy .;
						fi

						if [[ ! -f ./balf_clade_simData.PF.partitions ]]; then
							MY_TOTAL_NUM_LINES="$(cat ../PartitionFinder/analysis/best_scheme.txt | wc -l | sed 's/\ //g')"
							MY_RAXML_PART_START_LINE="$(cat best_scheme.txt | grep -n 'RaxML' | sed 's/:.*//')"
							sed -n ''"$MY_RAXML_PART_START_LINE"','"$MY_TOTAL_NUM_LINES"'p' ../PartitionFinder/analysis/best_scheme.txt > ./balf_clade_simData.PF.partitions
						fi
...
raxml -f a -x $(python -c "import random; print random.randint(10000,100000000000)") -p $(python -c "import random; print random.randint(10000,100000000000)") -# 100 -m GTRGAMMA -s ./balf_clade_simData.phy -q ./balf_clade_simData.PF.partitions -n balf_${MY_FOLDERNAME}_raxml
...
" > ./raxml_qsub.sh
					...
					echo "WARNING!  | $(date) | PartitionFinder run for ${MY_FOLDERNAME} is incomplete. Moving on to next run folder... "
				fi
			)
			cd ..;
		done
